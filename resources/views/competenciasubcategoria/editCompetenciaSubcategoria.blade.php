...
<x-plantilla-body>
    ...
    <script>    
        ...
        document.getElementById('min_participantes').addEventListener('input', function() {
            var maxParticipantes = document.getElementById('max_participantes');
            if (this.value.trim() !== '') {
                maxParticipantes.removeAttribute('disabled');
                maxParticipantes.value = ""; // Limpia el valor de fecha_fin si se deshabilita
            } else {
                maxParticipantes.setAttribute('disabled', 'true');
                maxParticipantes.value = ""; // Limpia el valor de fecha_fin si se deshabilita
            }
        });

        
        const minParticipantesInput = document.getElementById('min_participantes');
        const maxParticipantesInput = document.getElementById('max_participantes');

        // Actualizar el mínimo de fecha_fin al cambiar fecha
        minParticipantesInput.addEventListener('change', function () {
            if (minParticipantesInput.value) {                                                
                maxParticipantesInput.min = minParticipantesInput.value;
            } else {                
                maxParticipantesInput.min = 1;          
            }
        });
