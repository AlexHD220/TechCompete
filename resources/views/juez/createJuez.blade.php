<x-guest-layout>
    <x-authentication-card-register>
        ...
        <div class="flex items-center justify-center">
         <b><h1 style="margin-bottom: 15px; font-size: 20px;">Crea una cuenta como Juez</h1></b>
        </div>

        <form id="registroForm" method="POST" action="{{ route('juez.store') }}">
            @csrf

            <div class="mt-4">
                <x-label for="codigo" value="{{ __('Código de registro de cuenta') }}" />
                <x-input id="codigo" class="block mt-1 w-full" type="text" placeholder="15 digitos" name="codigo" :value="old('codigo')" minlength="15" maxlength="15" required autofocus /> <!-- autocomplete="name" --->
            </div>

            <div class="mt-4">
                <x-label for="email" value="{{ __('Correo electrónico') }}" />
                <x-input id="email" class="block mt-1 w-full" type="email" placeholder="Correo vinculado" name="email" :value="old('email')" minlength="5" maxlength="50" required /> <!-- autocomplete="username" -->
            </div>
            ...
            <div class="mt-4">
                ...
                <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" minlength="3" maxlength="20" required /> <!-- autocomplete="name" --->
            </div>
            ...
            <input id="codigo_registro" type="hidden" name="codigo_registro" value="{{ $codigo }}">
